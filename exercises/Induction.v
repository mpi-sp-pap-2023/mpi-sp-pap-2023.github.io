...
(** For this [Require] command to work, Coq needs to be
    able to find a compiled version of the previous chapter [Basics.v].
    This compiled version is called [Basics.vo] and is
    ...
    the [Basics.v], [Induction.v], and the [_CoqProject] files are in
    the same directory. They were all provided to you or you can use
    your own versions, but if you wish you can also create the
    [_CoqProject] file yourself containing only the following line:

      -Q . LF

    This maps the current directory ("[.]", which contains [Basics.v],
    [Induction.v], etc.) to the prefix (or "logical directory")
    "[LF]". Proof General, CoqIDE, VSCoq read [_CoqProject]
    ...
    build [Basics.vo] from an IDE (or you can alternatively follow the
    command line instructions below):
     ...
     - For VSCode users, open the terminal pane at the bottom 
       and then follow the command line instructions below.

    Alternatively, you can compile [Basics.v] from the command line:
     ...
     - One way to do this is by manually running this command:
       ...
       current file, in the right order, this is why for later
       chapters using the [make] solution below may be preferable.

     - If you have [make] installed on your machine you can use that
       to automatically compile dependencies.  You can compile
       [Basics.v] by running [make] with the corresponding [.vo] file
       as a target:
...
(** **** Exercise: 4 stars, advanced, optional (bin_nat_bin) *)
