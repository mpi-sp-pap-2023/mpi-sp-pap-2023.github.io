...
(** In some situations, it can be necessary to generalize a statement for being able to prove it by induction. 
Intuitively, the reason for this is that a more general statement also yields a more general (and hence stronger) inductive hypothesis. 
For this reason, when being stuck in a proof, it can make sense to step back and see whether it is possible to prove a stronger statement. *)
...
(** A first attempt may be to prove exactly the statement that we are missing at this point. 
However, we will fail to to so since the inductive hypothesis resulting from it is not general enough to conclude the proof. *)
...
(** This generalized lemma  would be sufficient to conclude our original proof. 
Still, we could prove an even more general lemma about the length of appended lists.  *)
...
(** To illustrate how induction on more complex datatypes works, we will introduce the datatype [btree] of labeled trees that are at most binary: 
This means that a each node in tree is labeled with a natural number and has at most two children.
We can describe such a datatype as follows: *)
...
Fixpoint sumLabels (t: btree): nat := 
  ...
  | binary n t1 t2 => n + sumLabels t1 + sumLabels t2
  end.
...
(** We introduce a helper lemma for doing a specific arithmetic transformation 
that we will use in the Theorem that we want to prove next. 
It is only requires a combination of associativity and commutativity of addition
but since it involves several of such steps it makes the main proof to have it as a seperate lemma. *)

Theorem transformEquationHelp: forall (a b c d e: nat), 
  (a + (b + c)) + (d + e) =  (b + d) + ((a + c) + e).
Proof.
  intros a b c d e.
...
(** We can state the following theorem on the sum of labels of a tree with incremented labels. 
The proof proceeds by induction on the tree [t]. 
Consequently, we will need to to prove three cases: 
...
Theorem increment_sumLabels: forall (t: btree), 
